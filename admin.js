...
async function loadStats() {
    try {
        const response = await fetch("https://skiza-backend.onrender.com/stats");
        const data = await response.json();

        document.getElementById("totalUploads").textContent = data.totalUploads;
    } catch (error) {
        document.getElementById("totalUploads").textContent = "Error";
    }
}
...
async function loadUploads() {
    try {
        const response = await fetch("https://skiza-backend.onrender.com/uploads");
        const uploads = await response.json();

        const gallery = document.getElementById("gallery");

        if (!uploads.length) {
            gallery.innerHTML = "<p>No screenshots uploaded yet.</p>";
            return;
        }
